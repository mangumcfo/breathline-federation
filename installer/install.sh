...
REPO_BRANCH="main"
VERSION_TARGET="v0.5.3"   # advertised target (banner only); state-file uses cloned manifest version
SKIP_BOOTSTRAP=0
SKIP_BREATH_GATE=0
...
write_node_state() {
  ...
  # v0.5.3: read installed version from the cloned manifest.yaml rather than
  # the hardcoded VERSION_TARGET (which was drifting behind tagged releases —
  # state-file showed v0.4.1 even when v0.5.2 was cloned).
  local manifest_version
  if [[ -f "$PREFIX/manifest.yaml" ]]; then
    manifest_version="$(grep '^version:' "$PREFIX/manifest.yaml" | head -1 | awk -F'"' '{print $2}')"
  fi
  manifest_version="${manifest_version:-$VERSION_TARGET}"   # fallback to advertised target
  cat > "$statefile" <<EOF
